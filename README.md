...
`CI_CD_DISPLAY_NAME` — номер заявки, например `ACDUU-490`.
Разрешены латинские буквы, цифры, `-` и `_`.
`-` и `_` не должны быть первым или последним символом.
`-` и `_` не должны повторяться подряд.

`project_name` — человекочитаемое название проекта.
`CI_CD_AUTHOR` — e-mail автора; проверяется общий формат e-mail.
`CI_CD_DEPARTMENTS` — список отделов через запятую.
Пример: `Отдел 1, Группа 2`.
Пустое значение разрешено и будет записано как `[]`.
Непустой список будет записан как JSON-массив.
`CI_CD_DESCRIPTION` — отдельное описание автоматизации до 255 символов.
